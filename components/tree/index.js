...
var Tree = function (_React$Component) {
	...
	function Tree(props) {
		...
		_this.handleNodeBlur = _this.handleNodeBlur.bind(_this);
		_this.handleExpandClick = _this.handleExpandClick.bind(_this);
		_this.state = {
			flattenedNodes: _this.flattenTree({
	...
	_createClass(Tree, [{
		...
		value: function flattenTree(root) {
			...
			if (!root.nodes) {
				return [{ node: root, treeIndex: treeIndex }];
			}
			var nodes = [{ node: root, treeIndex: treeIndex }];
			if (root.expanded) {
		...
		key: 'handleClick',
		value: function handleClick(event, data, clearSelectedNodes) {
			// When triggered by a key event, other nodes should be deselected.
			if (clearSelectedNodes) {
				this.state.flattenedNodes.forEach(function (flattenedNode) {
			...
			var selectedNodeIndexes = void 0;
			if (data.select) {
				selectedNodeIndexes = this.state.selectedNodeIndexes.concat([data.treeIndex]);
			} else {
				selectedNodeIndexes = this.state.selectedNodeIndexes.filter(function (treeIndex) {
		...
		key: 'handleNodeBlur',
		value: function handleNodeBlur() {
			this.setState({ treeHasFocus: false });
		}
	}, {
		key: 'handleExpandClick',
		value: function handleExpandClick(event, data) {
