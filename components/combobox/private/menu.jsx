...
const propTypes = {
	 ...
	 *
	 */
	menuItemSeparator: PropTypes.func,
	/*
	 * Menu options
	 */
...
const Menu = (props) => {
	const menuOptions = props.options.map((optionData, index) => {
		...
		const MenuItemSeparator = props.menuItemSeparator;

		if (optionData.type === 'separator') {
			const defaultSeparator = optionData.label ?
				(<span className="slds-text-title_caps">{ optionData.label }</span>)
